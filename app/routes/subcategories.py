...
@router.patch(
    "/edit/{subcategory_id}", response_model=ResponseSchema[SubCategorySchema]
)
async def edit_subcategory(
    subcategory_id: UUID,
    body: SubCategoryEditSchema,
    service: SubCategoryService = Depends(get_subcategory_service),
):
...
async def delete_subcategory(
    subcategory_id: UUID, service: SubCategoryService = Depends(get_subcategory_service)
):
